...
def normalized_colormap_function(simulation_time, time_step, cm_style=cm.bwr):
    simulation_time_list = np.arange(0, simulation_time + time_step, time_step)
    normalize = mcolors.Normalize(
        vmin=simulation_time_list.min(), vmax=simulation_time_list.max()
    )
    colormap = cm_style
...
def fill_function(
    ax, x, y, plot_color=None, alpha_value=None, x_bounds=None, y_bounds=None
):
